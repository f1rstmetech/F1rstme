...
const RSS_FEED = 'https://www.theverge.com/rss/index.xml';
...
async function loadLiveFeed() {
  ...
  try {
    ...
    el.innerHTML = json.items.slice(0, 3).map(item => `
      <article class="news-card">
        <span class="news-date">${escapeHtml(new Date(item.pubDate).toDateString())}</span>
        <h3>${escapeHtml(item.title)}</h3>
        <p>${escapeHtml((item.description || '').replace(/<[^>]+>/g, '').slice(0, 110))}…</p>
